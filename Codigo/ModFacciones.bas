...
Call WriteChatOverHead(UserIndex, "¡¡¡Bienvenido a al Ejercito Imperial!!!, aqui tienes tus vestimentas. Por cada centena de criminales que acabes te daré un recompensa, buena suerte soldado!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
...
If UserList(UserIndex).Faccion.CriminalesMatados \ 100 = _
   UserList(UserIndex).Faccion.RecompensasReal Then
    Call WriteChatOverHead(UserIndex, "Ya has recibido tu recompensa, mata 100 crinales mas para recibir la proxima!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
Else
    Call WriteChatOverHead(UserIndex, "Aqui tienes tu recompensa noble guerrero!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
...
If Not criminal(UserIndex) Then
    Call WriteChatOverHead(UserIndex, "Largate de aqui, bufon!!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
    Exit Sub
End If
...
If UserList(UserIndex).Faccion.ArmadaReal = 1 Then
    Call WriteChatOverHead(UserIndex, "Las sombras reinaran en Argentum, largate de aqui estupido ciudadano.!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
    Exit Sub
End If
...
If UserList(UserIndex).Faccion.RecibioExpInicialReal = 1 Then 'Tomamos el valor de ahí: ¿Recibio la experiencia para entrar?
    Call WriteChatOverHead(UserIndex, "No permitiré que ningún insecto real ingrese ¡Traidor del Rey!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
    Exit Sub
End If
'[/Barrin]

If Not criminal(UserIndex) Then
    Call WriteChatOverHead(UserIndex, "Ja ja ja tu no eres bienvenido aqui!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
    Exit Sub
End If
...
Call WriteChatOverHead(UserIndex, "Bienvenido a al lado oscuro!!!, aqui tienes tu armadura. Por cada centena de ciudadanos que acabes te daré un recompensa, buena suerte soldado!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
...
If UserList(UserIndex).Faccion.CiudadanosMatados \ 100 = _
   UserList(UserIndex).Faccion.RecompensasCaos Then
    Call WriteChatOverHead(UserIndex, "Ya has recibido tu recompensa, mata 100 ciudadanos mas para recibir la proxima!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
Else
    Call WriteChatOverHead(UserIndex, "Aqui tienes tu recompensa noble guerrero!!!", str(Npclist(UserList(UserIndex).flags.TargetNPC).Char.CharIndex), vbWhite)
    UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.Exp + ExpX100
    If UserList(UserIndex).Stats.Exp > MAXEXP Then _
