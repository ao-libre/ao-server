...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If .Faccion.ArmadaReal = 1 Then
            ...
            If TalkToDemon Then
                Call WriteChatOverHead(UserIndex, "Sal de aqui bufon!!!", _
                                       Npclist(NpcIndex).Char.CharIndex, vbWhite)
            
            Else
                ' Si le pidio al rey salir de la armada, le responde.
                If TalkToKing Then
                    Call WriteChatOverHead(UserIndex, "Seras bienvenido a las fuerzas imperiales si deseas regresar.", _
                                           Npclist(NpcIndex).Char.CharIndex, vbWhite)
                End If
        ...
        ElseIf .Faccion.FuerzasCaos = 1 Then
            ...
            If TalkToKing Then
                Call WriteChatOverHead(UserIndex, "Sal de aqui maldito criminal!!!", _
                                       Npclist(NpcIndex).Char.CharIndex, vbWhite)
            Else
                ' Si le pidio al demonio salir del caos, este le responde.
                If TalkToDemon Then
                    Call WriteChatOverHead(UserIndex, "Ya volveras arrastrandote.", _
                                           Npclist(NpcIndex).Char.CharIndex, vbWhite)
                End If
        ...
        Else
            ...
            If (TalkToDemon And criminal(UserIndex)) Or (TalkToKing And Not criminal(UserIndex)) Then 'Si se pueden unir a la faccion (status), son invitados
                Call WriteChatOverHead(UserIndex, "No perteneces a nuestra faccion. Si deseas unirte, di /ENLISTAR", _
                                       Npclist(NpcIndex).Char.CharIndex, vbWhite)
                        ...
                        If val(Arg1) >= 25 Then
                            ...
                            If GI > 0 Then
                                If modGuilds.GuildAlignment(GI) = "Del Mal" Or modGuilds.GuildAlignment(GI) = "Real" Then
                                    ...
                                    If tUser > 0 Then _
                                        Call WriteConsoleMsg(tUser, "Ya tienes la madurez suficiente como para decidir bajo que estandarte pelearas! Por esta razon, hasta tanto no te enlistes en la faccion bajo la cual tu clan esta alineado, estaras excluido del mismo.", FontTypeNames.FONTTYPE_GUILD)
                                End If
                            End If
                        End If
            ...
            Else
                ...
                If PersonajeExiste(UserName) Then
                    Call LogGM(.Name, " borro la pena: " & punishment _
                        & " de " & UserName & " y la cambio por: " & NewText)
