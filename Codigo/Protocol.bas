...
#End If
    ...
    ElseIf Err.Number <> 0 And Not Err.Number = UserList(UserIndex).incomingData.NotEnoughDataErrCode Then
        ...
        Call LogError("Error: " & Err.Number & " [" & Err.description & "] " & " Source: " & Err.source & _
                        ...
                        vbTab & " LastDllError: " & Err.LastDllError & vbTab & " - producido al manejar el paquete " & CStr(packetID))
        Call CloseSocket(UserIndex, True)
            ...
            Else
                ...
                If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                    Call LogGM(.name, " borro la pena: " & punishment & "-" & _
                      ...
                      & " de " & UserName & " y la cambió por: " & NewText, .flags.Privilegios And PlayerType.Consejero)
...
ErrHandler:
    If Err.error = UserList(UserIndex).outgoingData.NotEnoughSpaceErrCode Then
        Call FlushBuffer(UserIndex)
        Resume
    End If
End Sub
