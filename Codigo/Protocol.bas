    ...
    With UserList(UserIndex)
        ...
        If Not (EsDios(UserName) Or EsAdmin(UserName)) Or .flags.Privilegios >= PlayerType.Dios Then
            ...
            Else
                ...
                Call LogGM(.name, "/IRA " & UserList(tUser).name & " Mapa:" & UserList(tUser).Pos.Map & " X:" & UserList(tUser).Pos.X & " Y:" & UserList(tUser).Pos.Y, (.flags.Privilegios = PlayerType.Consejero))
...
'***************************************************
    With UserList(UserIndex)
        ...
        Call LogGM(.name, "/INVISIBLE", (.flags.Privilegios = PlayerType.Consejero))
    End With
End Sub
...
'***************************************************
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then _
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        For i = 1 To LastUser
            If (UserList(i).name <> "") And UserList(i).Counters.Trabajando > 0 Then
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        For i = 1 To LastUser
            If (UserList(i).name <> "") And UserList(i).Counters.Ocultando > 0 Then
                users = users & UserList(i).name & ", "
            End If
        Next i
...
'***************************************************
    With UserList(UserIndex)
        ...
        If .flags.Privilegios = PlayerType.Consejero And .Pos.Map = MAPA_PRETORIANO Then
...
'***************************************************
    With UserList(UserIndex)
        ...
        For LoopC = 1 To LastUser
            If UserList(LoopC).name <> "" And UserList(LoopC).Pos.Map = .Pos.Map And (UserList(LoopC).flags.Privilegios < PlayerType.Dios Or .flags.Privilegios >= PlayerType.Dios) Then
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            ...
            If tUser <= 0 Then
                ...
                If FileExist(CharPath & UserName & ".chr", vbNormal) Then
                    ...
                    If userPriv > .flags.Privilegios Then
                        Call WriteConsoleMsg(UserIndex, "No podes banear a al alguien de mayor jerarquia.", FontTypeNames.FONTTYPE_INFO)
                    Else
                        ...
                        Else
                            Call LogBanFromName(UserName, UserIndex, reason)
            ...
            Else
                ...
                Call LogBan(tUser, UserIndex, reason)
...
On Error GoTo errhandler
    ...
    With UserList(UserIndex)
        ...
        Else
            If .flags.Privilegios = PlayerType.Dios Or _
              UserList(tUser).flags.Privilegios <= PlayerType.Consejero Then
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            ...
            Else
                ...
                With UserList(tUser)
                    .flags.PertAlCons = 1
                    '*Nigo: por las dudas lo expulso del ConsejoCaos
                    .flags.PertAlConsCaos = 0
                    '*Nigo: si bien funciona, es medio pedorro hacer esto xD
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            ...
            Else
                ...
                With UserList(tUser)
                    '*Nigo: por las dudas lo expulso del ConsejoReal
                    .flags.PertAlCons = 0
                    .flags.PertAlConsCaos = 1
                    '*Nigo: si bien funciona, es medio pedorro hacer esto xD
                    Call WarpUserChar(tUser, .Pos.Map, .Pos.X, .Pos.Y, False)
                End With
            End If
...
On Error GoTo errhandler
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            ...
            Else
                '*Nigo: no se como reemplazar esto :(
                Call SendData(SendTarget.ToIndex, tUser, 0, "DUMB")
            End If
        End If
...
On Error GoTo errhandler
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            ...
            Else
                '*Nigo: no se como reemplazar esto :(
                Call SendData(SendTarget.ToIndex, tUser, 0, "NESTUP")
            End If
        End If
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            If tUser <= 0 Then
                ...
                Else
                    '*Nigo: mandamos la ruta entera? yo mandaria unicamente el nombre del usuario...
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If tTrigger >= 0 Then
            With MapData(.Pos.mapa, .Pos.X, .Pos.Y)
                .trigger = tTrigger
                tLog = "Trigger " & .trigger & " en mapa " & .Pos.mapa & " " & .Pos.X & "," & .Pos.Y
            End With
        ...
        If Not .flags.EsRolesMaster Then
            ...
            Else
                ...
                For LoopC = 1 To cantMembers
                    ...
                    'ponemos la pena
                    tIndex = val(GetVar(CharPath & member & ".chr", "PENAS", "Cant"))
                    Call WriteVar(CharPath & member & ".chr", "PENAS", "Cant", tIndex + 1)
                    Call WriteVar(CharPath & member & ".chr", "PENAS", "P" & tIndex + 1, LCase$(.name) & ": BAN AL CLAN: " & GuildName & " " & Date & " " & time)
                Next LoopC
            End If
        End If
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        Dim banedIP As Long
        banedIP = .incomingData.ReadLong
        
        '*Nigo: me mareas manejando las ips juanma xD (hacelo vos este)
    End With
End Sub
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If ObjData(tObj).name = "" Then Exit Sub
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If IsNumeric(midiID) Then
            ...
            '*Nigo: no se como se reemplaza esto
            Call SendData(SendTarget.ToAll, 0, 0, "TM" & midiID)
        End If
    End With
End Sub
...
'***************************************************
    ...
    With UserList(UserIndex)
        ...
        If IsNumeric(waveID) Then
            '*Nigo: no se como se reemplaza esto
            Call SendData(SendTarget.ToAll, 0, 0, "TM" & waveID)
        End If
    End With
End Sub
...
On Error GoTo errhandler
    ...
    With UserList(UserIndex)
        ...
        If Not .flags.EsRolesMaster Then
            If UserName = "" Or Not IsNumeric(punishment) Then
...
'***************************************************
    With UserList(UserIndex)
        ...
        With MapData(.Pos.Map, .Pos.X, .Pos.Y)
            If .Blocked = 0 Then
                .Blocked = 1
            Else
                .Blocked = 0
            End If
            '*Nigo esto queda asi?
            Call Bloquear(SendTarget.ToMap, UserIndex, .Pos.Map, .Pos.Map, .Pos.X, .Pos.Y, .Blocked)
        End With
    End With
End Sub
